...
hexo.extend.tag.register('AmazonJpLink2', (args) => {
  const asin = args[0];
  const tag = args[1];
  const text = args[2];
  let size = args[3];
  if (!size) { size = 150; }
  return `<img border="0" src="http://ws.assoc-amazon.jp/widgets/q?_encoding=UTF8&ASIN=${asin}&Format=_SL${size * 2}_&ID=AsinImage&MarketPlace=JP&ServiceVersion=20070822&WS=1&tag=${tag}" width="${size}" style="float: left; margin: 0 20px 20px 0;" >`
        + `<a href="http://www.amazon.co.jp/gp/product/${asin}/ref=as_li_qf_sp_asin_il?ie=UTF8&camp=247&creative=1211&creativeASIN=${asin}&linkCode=as2&tag=${tag}" target="_blank">${text}</a>`;
});
